...
def generate_device_cell(IDT_aperature, prop_length,periods_num, origin=(50, -25)):
    # Create ZnO
    ...
    how_many_period = int(periods_num)
    ...
    Finger_origin_x = 49
    Finger_origin_y = -8
    Finger_left_offset = prop_length  # two pair IDTs' distance offset =Standard:298)
    Finger_length = IDT_aperature

    arm2_right_pad_Offset = Finger_length + 4.5
    pad_length = how_many_period * 2  # Normal finger * 2 , split finger * 4
    ...
    #Creat the trenches
        ...
        # bot right trench (port starts from right to left )
    wg1 = Waveguide.make_at_port(Port(origin, angle=np.pi, width= Trench_width))
    wg1.add_straight_segment(length=prop_length)

        # Right reflector (port starts from bot to top )
    reflector1 = Waveguide.make_at_port(Port((100 + pad_length/4,-15), angle=np.pi/2, width= Trench_width))
    reflector1.add_straight_segment(length=IDT_aperature+10)

        # top right trench (port starts from right to left )
    wg2 = Waveguide.make_at_port(Port((50,IDT_aperature + 2), angle=np.pi, width= Trench_width))
    wg2.add_straight_segment(length=prop_length)

        # left reflector (port starts from bot to top )
    reflector2 = Waveguide.make_at_port(Port((-prop_length - pad_length/4,-15), angle=np.pi / 2, width=Trench_width))
    reflector2.add_straight_segment(length=IDT_aperature+10)
    ...
    Finger_lower = Waveguide.make_at_port(
        Port(origin=(Finger_origin_x + pad_length/4 , Finger_origin_y - 5), angle=np.pi / 2, width=Idt_finger_arm2))
    Finger_lower.add_straight_segment(length=Finger_length)

    Finger_upper = Waveguide.make_at_port(
        Port(origin=(Finger_origin_x + Figer_gap_offset + pad_length/4, Finger_origin_y), angle=np.pi / 2, width=Idt_finger_arm2))
    ...
    Finger_lower_other_side = Waveguide.make_at_port(
        Port(origin=(Finger_origin_x - Finger_left_offset - pad_length/4, Finger_origin_y - 5), angle=np.pi / 2,
             width=Idt_finger_arm2))
    Finger_lower_other_side.add_straight_segment(length=Finger_length)

    Finger_upper_other_side = Waveguide.make_at_port(
        Port(origin=(Finger_origin_x - Finger_left_offset + Figer_gap_offset - pad_length/4, Finger_origin_y), angle=np.pi / 2,
             width=Idt_finger_arm2))
    Finger_upper_other_side.add_straight_segment(length=Finger_length)

    #Make Small metal pad
    outer_corners_arm2_1 = [
        (Finger_origin_x + pad_length * (figer_width + pitch)/3 - Finger_left_offset, Finger_origin_y - 10),
        (Finger_origin_x + pad_length * (figer_width + pitch)/3 - Finger_left_offset, Finger_origin_y - 5),
    ...
    outer_corners_arm2_2 = [(Finger_origin_x + pad_length * (figer_width + pitch)/3 - Finger_left_offset,
                             Finger_origin_y + arm2_right_pad_Offset),
                            (Finger_origin_x + pad_length * (figer_width + pitch)/3 - Finger_left_offset,
    ...
    outer_corners_arm1_1 = [(Finger_origin_x - pad_length * (figer_width + pitch)/3, Finger_origin_y - 10),
                            (Finger_origin_x - pad_length * (figer_width + pitch)/3, Finger_origin_y - 5),
    ...
    outer_corners_arm1_2 = [
        (Finger_origin_x - pad_length * (figer_width + pitch)/3, Finger_origin_y + arm2_right_pad_Offset),
        (Finger_origin_x - pad_length * (figer_width + pitch)/3, Finger_origin_y + arm2_right_pad_Offset - 5),
    ...
    #Make_ZnO_Pad
    Outer_corner_ZnO_pad_R = [(ZnO_Original + pad_length/2 + ZnO_expand_x , -15 - ZnO_expand_y),
                            (ZnO_Original + pad_length/2 + ZnO_expand_x, -15 + IDT_aperature + 10 + ZnO_expand_y),
                            (ZnO_Original - pad_length/2 - ZnO_expand_x, -15 + IDT_aperature + 10 + ZnO_expand_y),
                            (ZnO_Original - pad_length/2 - ZnO_expand_x, -15 - ZnO_expand_y)]
    ...
    #Add Cell
    cell = Cell('SIMPLE_RES_DEVICE r={:.1f} g={:.1f}'.format(IDT_aperature, periods_num))

    cell.add_to_layer(1, wg1, reflector1, wg2, reflector2)
    ...
    cell.add_ebl_marker(layer=1, marker=CrossMarker(origin=(-300,-300 ), cross_length=10 , cross_width=5, paddle_length=5, paddle_width=5))
...
layout = GridLayout(title='GaP on Sapphire', frame_layer=0, text_layer=2, region_layer_type=None, horizontal_spacing=100, vertical_spacing=0)
IDT_aperatureS = np.linspace(20, 200, 7)
prop_length = 100
periods_nums = np.linspace(15,60,10)
total = len(IDT_aperatureS) * len(periods_nums)
count = 0
# Add column labels
layout.add_column_label_row(('period num %0.2f' % periods_num for periods_num in periods_nums), row_label='')
...
if show == False:
    for IDT_aperature in IDT_aperatureS:
        layout.begin_new_row('AP=\n%0.2f' % IDT_aperature)
        for periods_num in periods_nums:
            count =  count + 1
            complete = count/total
            print("Number of cell generated / Total cell = %0.1f/%0.1f (%0.2f%% complete) " %(count ,total,complete*100) )
            layout.add_to_row(generate_device_cell(IDT_aperature, prop_length, periods_num ), alignment='center-center' , realign=True)
...
if show == False:
    print('saving........')
    layout_cell.save('ZnO-TEST2.gds')
    print('saved!!!')